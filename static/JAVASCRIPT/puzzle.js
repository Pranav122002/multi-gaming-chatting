$(document).ready(function() {     
	 //The ready() method is used to make a function
	 // available after the document is loaded.Whatever code you write inside the $(document ).ready()
	 // method will run once the page DOM is ready to execute JavaScript code


	var box = $(".box"),  // declare variable
	 // Declare no of boxes total 18
		...
		ss = 0,
		upIMG,
		images = ["/static/PHOTOS/puzzle/naruto.jpg","/static/PHOTOS/puzzle/minion.jpg","/static/PHOTOS/puzzle/tom_jerry.jpg"]   // demo images
		img = 0;




	$('.me').css({"background-image" : 'url('+images[0]+')'});  // set background image

	$(".start").click(function() {                   // start button starts the game
		$(".start").addClass('prevent_click');
		$(".start").delay(100).slideUp(500);
		$(".full").hide();
		$(".pre_img").addClass("prevent_click");    //on mouse hover shows game image
		
		date1 = new Date();         // importing date 
		Start();
		return 0;
	});

	function Start() {  //starts the game and selects random tiles
		randomTile();
		changeBG(img);
		var count = 0,
			a,
		...
		$(".me").click(function() {
			...
			if (count == 1) {
				...
				moves++;
				swapping(a, b);         // swaping image boxes checks if the box is correct with the image
				checkCorrect(a);        // then fix that box
				checkCorrect(b);
				a = b = count = A = B = 0;
			}
			if (arraysEqual(x)) {      // shows score and total time taken
				date2 = new Date();
				timeDifferece();
				showScore();
				return 0;
	...
	function randomTile() {         // create random box when click on start
		var i;
